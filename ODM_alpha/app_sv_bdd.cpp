...
void app_sv_bdd::start(){
    QThread::sleep(1);
    if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << "Start of app_sv_bdd on:";
    if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << Q_FUNC_INFO << QThread::currentThreadId();
    ...
    if(db.open())
    {
        if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << "Connected to:";
        if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << db.hostName();
        is_running = true;
        if(this->init())
            this->loop();
    }
    ...
    {
        if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << "Connecion failed";
        if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << db.lastError().text();
    }
    db.close();
}
    ...
    while(is_running){
        if(updateState()){
            for(int i=0 ; i<state.length() ; i++){
                ...
                if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << "INSERT INTO `main`(`"+TABLE_ID+"`, `"+TABLE_LONG+"`, `"+TABLE_LAT+"`, `"+TABLE_ALT+"`, `positiondate`) VALUES ("+id_drone+","+longitude+","+latitude+","+altitude+",\""+date+"\")";
                ...
                {
                    if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << "Insert success";
                }
                else{
                    if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << "Insert Failed !";
                    if (DEBUG_ENABLE) qDebug() << MARQUEUR_DEBUG << query.lastError().text();
                }
            }
        }
